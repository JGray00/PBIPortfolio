...
```
schema to be added once finalised, current schema can be found in the .pbix file
```
...
In addition to the SQL queries, a date reference table was created as well as an additional table for the purposes of filtering by a given month. These were created with DAX functions rather than in SQL due to the built in functions, however if a date reference table was in the database, then that could also be used. The DAX for these is given in the DAX formula file. The second table was mostly required due to the fact that this is not current data. If it were current data then a relative date filter could be used instead.

At present, the visualisations done are only related to a handful of tables and will be updated as further work is done.
